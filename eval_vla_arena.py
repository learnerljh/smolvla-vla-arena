    ...
    for task_id in tqdm(range(num_tasks_in_suite), desc="Tasks"):
        ...
        for episode_idx in tqdm(
            ...
            if should_save_video:
                ...
                video_path = (
                    pathlib.Path(video_out_path) / f"{TIME}_rollout_task_{task_id}_episode_{episode_idx}_{task_segment}_{suffix}.mp4"
                )
                fps = 30
...
def _get_vla_arena_env(task, resolution, seed,add_noise=False,randomize_color=False,adjust_light=False,camera_offset=False):
    ...
    task_bddl_file = pathlib.Path(get_vla_arena_path("bddl_files")) / task.problem_folder / f"level_{task.level}" / task.bddl_file
    env_args = {
